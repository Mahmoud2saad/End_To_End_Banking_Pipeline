...
│           └──────────────┘   └─────────────────────────┘   └──────────────────┘  │
└──────────────────────────────────────────────────────────────────────────────────┘
See architecture.png, schema.png, and modeling.png for full visual diagrams.
...
Provision Azure Data Lake Storage Gen2 with hierarchical namespace enabled
Create containers: landing, bronze, silver, gold
Create a Databricks workspace and configure a cluster with Delta Lake and dbt installed
Set all AZURE_* and DATABRICKS_* variables in .env
DAGs 01_bronze_cloud.py and 02_silver_cloud.py run as Databricks jobs via the databricks-sdk
dbt targets Databricks via dbt-databricks — same models, same tests
Snowflake Integration
...
*Built by Mahmoud Saad · Alfred Farag . Mariam Safwat .Zainab Mohamed *
